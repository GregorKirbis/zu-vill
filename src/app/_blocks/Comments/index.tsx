      ...
      <Gutter>
        <div className={classes.comments}>
          ...
          {comments?.map((com, index) => {
            ...
            return (
              <Fragment key={index}>
                <div className={[classes.column, comments.length === 2 && classes["cols-half"], comments.length >= 3 && classes["cols-thirds"]].filter(Boolean).join(" ")}>
                  {_status === "draft" && (
                    <Message
                      message={
                        <Fragment>
                          {"This is a draft comment. You are seeing it because you are an admin. To approve this comment, "}
                          <Link href={`${process.env.NEXT_PUBLIC_SERVER_URL}/admin/collections/comments/${com.id}`}>navigate to the admin dashboard</Link>
                          {' and click "publish".'}
                        </Fragment>
                      }
                    />
