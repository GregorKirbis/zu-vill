...
export const CommentForm: React.FC<{
      ...
      try {
        ...
        setSuccess(
          <Fragment>
            ...
            <Link href={`${process.env.NEXT_PUBLIC_SERVER_URL}/admin/collections/comments/${typeof json.doc === "object" ? json.doc.id : json.doc}`}>navigate to the admin dashboard</Link>
            {' and click "publish".'}
          </Fragment>
        );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className={classes.form}>
      ...
      />
      {!user ? <Button href={`/login?redirect=${encodeURIComponent(pathname)}`} appearance="primary" label="Login to comment" disabled={isLoading} className={classes.submit} /> : <Button type="submit" appearance="primary" label={isLoading ? "Processing" : "Comment"} disabled={isLoading} className={classes.submit} />}
    </form>
  );
};
