...
export default async function Account() {
  const { user } = await getMeUser({
    nullUserRedirect: `/login?error=${encodeURIComponent("You must be logged in to access your account.")}&redirect=${encodeURIComponent("/account")}`,
  });
  ...
  return (
    <Fragment>
      ...
      <Gutter className={classes.account}>
        ...
        <HR />
        <h2>Comments</h2>
        <p>These are the comments you have placed over time. Each comment is associated with a specific post. All comments must be approved by an admin before they appear on the site.</p>
        <HR />
