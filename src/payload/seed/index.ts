...
export const seed = async (payload: Payload): Promise<void> => {
    ...
    posts.map(
      async (post, index) =>
        await payload.create({
          ...
          data: {
            _status: "published",
            comment: `This is a comment on post ${index + 1}. It has been approved by an admin and is now visible to the public. You can leave your own comment on this post using the form below.`,
            user: demoUserID,
            doc: post.id,
          },
        })
  ...
  const project1Doc = await payload.create({
    collection: "projects",
    data: JSON.parse(JSON.stringify({ ...project1, categories: [designCat.id] }).replace(/"\{\{IMAGE\}\}"/g, image2ID)),
  });

  const project2Doc = await payload.create({
    collection: "projects",
    data: JSON.parse(JSON.stringify({ ...project2, categories: [softwareCat.id] }).replace(/"\{\{IMAGE\}\}"/g, image2ID)),
  });

  const project3Doc = await payload.create({
    collection: "projects",
    data: JSON.parse(JSON.stringify({ ...project3, categories: [engineeringCat.id] }).replace(/"\{\{IMAGE\}\}"/g, image2ID)),
  });
